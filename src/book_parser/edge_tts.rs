...
//!
//! 目标：在 musl / Android 等环境也可用（避免 OpenSSL / native-tls / curl）。
...
// 这些值与 msedge-tts crate 的实现保持一致，以提升兼容性。
const USER_AGENT: &str = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36 Edg/120.0.0.0";
...
fn xml_escape(s: &str) -> String {
    // Very small escaping for SSML text node.
    // Avoid pulling in extra deps.
